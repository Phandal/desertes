    ...
    while (true) {
      ...
      switch (token.type) {
        case TokenType.Element_Separator: {
          const peekedToken = this.scanner.peekToken();
          if (peekedToken === null || (peekedToken.type === TokenType.Element_Separator || peekedToken.type === TokenType.Segment_Separator)) {
            currentSegment.processElement(new Token(TokenType.Element, ''));
          }
          break;
        }
