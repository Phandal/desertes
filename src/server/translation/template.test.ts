...
describe('loadSchema', () => {
	...
	it('should throw if the request failed', async () => {
		...
		assert.rejects(async () => loadSchemaHTTPS('localhost'), {
			name: 'Error',
	...
	it('should throw if the request was not successful', async () => {
		...
		assert.rejects(async () => loadSchemaHTTPS('localhost'), {
			name: 'Error',
			message: 'schema loading error: 400',
		});
	});
	...
	it('should throw if a JSON object was not returned', async () => {
		...
		assert.rejects(async () => loadSchemaHTTPS('localhost'), {
			name: 'Error',
			message: 'schema parsing error: Unexpected end of JSON input',
		});
	});
});
...
describe('validateSchema', () => {
	it('should return a valid response when it works', async () => {
		...
		const got = await validateSchema(template, {
			loadSchema: async () => {
				return {};
			},
		});
	...
	it('should return an error when the fetch fails', async () => {
		...
		const got = await validateSchema(template, {
			loadSchema: async () => {
				throw new Error('mock failed fetch');
			},
		});
		const want = new Error(
			"failed to fetch schema definition from 'localhost': mock failed fetch",
		);
